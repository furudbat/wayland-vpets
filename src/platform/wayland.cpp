...
namespace bongocat::platform::wayland {
    ...
    void update_config(wayland_session_t& ctx, const config::config_t& config, animation::animation_session_t& trigger_ctx) {
        ...
        const auto old_height = ctx.wayland_context._local_copy_config ? ctx.wayland_context._local_copy_config->overlay_height : 0;
        const auto old_width = ctx.wayland_context._screen_width;

        // update old config
        *ctx.wayland_context._local_copy_config = config;

        const bool dimensions_changed = (old_height != ctx.wayland_context._local_copy_config->overlay_height) || (ctx.wayland_context._local_copy_config->screen_width > 0 && old_width != ctx.wayland_context._local_copy_config->screen_width);
        ...
        if (dimensions_changed && old_height > 0 && old_width > 0 && ctx.wayland_context.ctx_shm) {
            ...
            BONGOCAT_LOG_INFO("Dimensions changed (%dx%d -> %dx%d), recreating buffer...", old_width, old_height, ctx.wayland_context._local_copy_config->screen_width, ctx.wayland_context._local_copy_config->overlay_height);
            ...
            if (ctx.wayland_context._screen_width > 0 && ctx.wayland_context._bar_height > 0) {
              ...
              BONGOCAT_LOG_ERROR("Buffer recreated failed (%dx%d)",
                                ctx.wayland_context._local_copy_config->screen_width,
                                ctx.wayland_context._local_copy_config->overlay_height);
            }
        }
