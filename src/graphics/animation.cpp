...
inline static constexpr time_ms_t POOL_MIN_TIMEOUT_MS = 5;
inline static constexpr time_ms_t POOL_MAX_TIMEOUT_MS = 1000;
...
static bool anim_handle_idle_return(animation_context_t& ctx, const input_context_t& input, const animation_state_t& state, timestamp_us_t current_time_us) {
    assert(input.shm);
...
static bool anim_update_state(animation_trigger_context_t& animation_trigger_ctx, animation_state_t& state) {
    ...
    const input_context_t& input = *animation_trigger_ctx._input;
    animation_context_t& ctx = *animation_trigger_ctx._anim;
    // read-only config
...
static void *anim_thread_main(void *arg) {
    ...
    while (atomic_load(&ctx._animation_running)) {
        ...
        ctx.shm->time_until_next_frame_ms =
            static_cast<time_ms_t>(sec_diff * 1000L + (nsec_diff + 999999LL) / 1000000LL);
