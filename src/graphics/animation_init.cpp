...
namespace bongocat::animation {
    ...
    bongocat_error_t init(animation_trigger_context_t& trigger_ctx, animation_context_t& ctx, const config::config_t& config) {
        ...
        ctx.shm = platform::make_allocated_mmap<animation_shared_memory_t>();
        if (ctx.shm == nullptr) {
        ...
        ctx._local_copy_config = platform::make_allocated_mmap<config::config_t>();
        ...
        trigger_ctx.render_efd = platform::FileDescriptor(eventfd(0, EFD_NONBLOCK | EFD_CLOEXEC));
        ...
        const auto result = anim_load_embedded_images_into_sprite_sheet(ctx.shm->anims[assets::BONGOCAT_ANIM_INDEX].sprite_sheet, bongocat_embedded_images, BONGOCAT_EMBEDDED_IMAGES_COUNT);
