...
embedded_image_t get_ms_agent_sprite_sheet(size_t i) {
  ...
  case CLIPPY_ANIM_INDEX:
    return {clippy_png, clippy_png_size, "clippy"};
#ifdef FEATURE_MORE_MS_AGENT_EMBEDDED_ASSETS
  case LINKS_ANIM_INDEX:
    return {links_png, links_png_size, "links"};
  case ROVER_ANIM_INDEX:
    return {rover_png, rover_png_size, "rover"};
  case MERLIN_ANIM_INDEX:
    return {merlin_png, merlin_png_size, "merlin"};
#endif
  default:
    return {};
  }
