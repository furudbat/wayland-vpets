...
bongocat_error_t init_pen_anim(animation_thread_context_t& ctx, int anim_index, const assets::embedded_image_t& sprite_sheet_image, int sprite_sheet_cols, int sprite_sheet_rows) {
    ...
    assert(anim_index >= 0);
    ctx.shm->pen_anims[static_cast<size_t>(anim_index)] = bongocat::move(result.result);
    assert(ctx.shm->pen_anims[static_cast<size_t>(anim_index)].type == animation_t::type_t::Dm);
