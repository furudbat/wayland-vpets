...
bongocat_error_t init_pen20_anim(animation_thread_context_t& ctx, size_t anim_index, const assets::embedded_image_t& sprite_sheet_image, int sprite_sheet_cols, int sprite_sheet_rows) {
    ...
    assert(anim_index < PEN20_ANIM_COUNT);
    BONGOCAT_LOG_VERBOSE("Load pen20 Animation (%d/%d): %s ...", anim_index, PEN20_ANIM_COUNT, sprite_sheet_image.name);
    ...
    ctx.shm->pen20_anims[anim_index] = bongocat::move(result.result);
    assert(ctx.shm->pen20_anims[anim_index].type == animation_t::type_t::Dm);
