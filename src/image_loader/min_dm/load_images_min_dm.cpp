...
bongocat_error_t init_min_dm_anim(animation_context_t& ctx, int anim_index,
  ...
  assert(anim_index >= 0 && static_cast<size_t>(anim_index) < MIN_DM_ANIM_COUNT);
  ...
  assert(anim_index >= 0);
  ctx.shm->min_dm_anims[static_cast<size_t>(anim_index)] = bongocat::move(result.result);
  assert(ctx.shm->min_dm_anims[static_cast<size_t>(anim_index)].type == animation_t::type_t::Dm);
