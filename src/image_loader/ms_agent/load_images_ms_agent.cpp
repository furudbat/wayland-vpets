...
namespace bongocat::animation {
    ...
    created_result_t<ms_agent_sprite_sheet_t> load_ms_agent_anim(const animation_context_t& ctx, [[maybe_unused]] int anim_index, const assets::embedded_image_t& sprite_sheet_image, int sprite_sheet_cols, int sprite_sheet_rows, const assets::ms_agent_animation_indices_t& animation_data) {
        ...
        // data not used
        using namespace assets;
        for (int col = result.result.idle.start_col,i = 0; col <= result.result.idle.end_col && static_cast<size_t>(i) < MAX_ANIMATION_FRAMES; col++,i++) {
            result.result._placeholder_animations.idle[i] = col;
        }
        for (int col = result.result.boring.start_col,i = 0; col <= result.result.boring.end_col && static_cast<size_t>(i) < MAX_ANIMATION_FRAMES; col++,i++) {
            result.result._placeholder_animations.boring[i] = col;
        }

        // full writing animation
        {
            size_t i = 0;
            for (int col = result.result.start_writing.start_col; col <= result.result.start_writing.end_col && i < MAX_ANIMATION_FRAMES; col++) {
                result.result._placeholder_animations.writing[i] = col;
                if (col < result.result.start_writing.end_col) i++;
            }
            for (int col = result.result.writing.start_col; col <= result.result.writing.end_col && i < MAX_ANIMATION_FRAMES; col++) {
                result.result._placeholder_animations.writing[i] = col;
                if (col < result.result.writing.end_col) i++;
            }
            for (int col = result.result.end_writing.start_col; col <= result.result.end_writing.end_col && i < MAX_ANIMATION_FRAMES; col++) {
                result.result._placeholder_animations.writing[i] = col;
                if (col < result.result.end_writing.end_col) i++;
            }
        }

        result.result._placeholder_animations.sleep[0] = result.result.sleep.end_col;
        result.result._placeholder_animations.sleep[1] = result.result.sleep.end_col;
        result.result._placeholder_animations.sleep[2] = result.result.sleep.end_col;
        result.result._placeholder_animations.sleep[3] = result.result.sleep.end_col;

        // full working animation
        {
            size_t i = 0;
            for (int col = result.result.start_working.start_col; col <= result.result.start_working.end_col && i < MAX_ANIMATION_FRAMES; col++) {
                result.result._placeholder_animations.working[i] = col;
                if (col < result.result.start_working.end_col) i++;
            }
            for (int col = result.result.working.start_col; col <= result.result.working.end_col && i < MAX_ANIMATION_FRAMES; col++) {
                result.result._placeholder_animations.working[i] = col;
                if (col < result.result.working.end_col) i++;
            }
            for (int col = result.result.end_working.start_col; col <= result.result.end_working.end_col && i < MAX_ANIMATION_FRAMES; col++) {
                result.result._placeholder_animations.working[i] = col;
                if (col < result.result.end_working.end_col) i++;
            }
        }

        // full moving animation
        {
            size_t i = 0;
            for (int col = result.result.start_moving.start_col; col <= result.result.start_moving.end_col && i < MAX_ANIMATION_FRAMES; col++) {
                result.result._placeholder_animations.moving[i] = col;
                if (col < result.result.start_moving.end_col) i++;
            }
            for (int col = result.result.moving.start_col; col <= result.result.moving.end_col && i < MAX_ANIMATION_FRAMES; col++) {
                result.result._placeholder_animations.moving[i] = col;
                if (col < result.result.moving.end_col) i++;
            }
            for (int col = result.result.end_moving.start_col; col <= result.result.end_moving.end_col && i < MAX_ANIMATION_FRAMES; col++) {
                result.result._placeholder_animations.moving[i] = col;
                if (col < result.result.end_moving.end_col) i++;
            }
        }

        for (int col = result.result.idle.start_col,i = 0; col <= result.result.idle.end_col && static_cast<size_t>(i) < MAX_ANIMATION_FRAMES; col++,i++) {
            result.result._placeholder_animations.happy[i] = col;
        }
