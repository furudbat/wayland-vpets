...
namespace bongocat::animation {
    bongocat_error_t init_dmc_anim(animation_thread_context_t& ctx, size_t anim_index, const assets::embedded_image_t& sprite_sheet_image, int sprite_sheet_cols, int sprite_sheet_rows) {
        ...
        assert(anim_index < DMC_ANIM_COUNT);
        ...
        ctx.shm->dmc_anims[anim_index] = bongocat::move(result.result);
        assert(ctx.shm->dmc_anims[anim_index].type == animation_t::type_t::Dm);
